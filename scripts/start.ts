...
async function startSession() {
  ...
  const DB_PATH = join(TARGET_REPO, ".aio", "aiorchestration.db");
  const orcheCmd = `DB_PATH=${DB_PATH} TARGET_REPO_ROOT=${TARGET_REPO} PROJECT_ROOT=${PROJECT_ROOT} GENERATED_DIR=${GENERATED_DIR} SESSION_UID=${UID} ORCHE_PANE=${orchePane} PLANNER_PANE=${plannerPane} bun run ${PROJECT_ROOT}/src/orche-process.ts`;
